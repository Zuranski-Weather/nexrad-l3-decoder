/**
 * bzip2 decompression for browser use.
 * Vendored from bz2 (C) 2019-present SheetJS LLC - MIT License
 * https://github.com/sheetjs/bz2
 *
 * Converted to ESM TypeScript module to avoid Vite CJS optimization issues.
 */
...
interface HuffmanTable {
  table: HuffmanEntry[];
  fastAccess: Record<number, HuffmanEntry>[];
}
...
function createOrderedHuffmanTable(lengths: number[]): HuffmanTable {
  ...
  let tempBits = 0;
  let symbol = -1;
  const fastAccess: Record<number, HuffmanEntry>[] = [];
  let current: Record<number, HuffmanEntry>;
...
function bwtReverse(src: number[], primary: number): number[] {
...
export function bz2Decompress(bytes: Uint8Array): Uint8Array {
  let index = 0;
  let bitfield = 0;
  let bits = 0;
  ...
  while (true) {
    ...
    if (blocktype === 0x314159265359) {
      ...
      const symbolsInUse = used.reduce((a, b) => a + (b ? 1 : 0), 0) + 2;
      ...
      let t: HuffmanTable;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- reused for HuffmanEntry and code number per original bz2 logic
      let r: any;
      let repeat = 0;
      let repeatPower = 0;
      const buffer: number[] = [];
      ...
      while (true) {
        ...
        for (const b in t!.fastAccess) {
          ...
          r = t!.fastAccess[bn][bitfield >> (bits - bn)];
          if (r) {
            bitfield &= masks[bits -= bn];
            r = r.code;
            break;
          }
        }
